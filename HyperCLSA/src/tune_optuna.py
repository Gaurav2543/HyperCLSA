...
def objective(trial):
    wandb.init(project="HyperCLSA", config=trial.params, reinit=True)
    ...
    fs_method   = trial.suggest_categorical("fs_method", ["boruta", "rfe"])
    if fs_method == "boruta":
    ...
    set_seed(42)  # reproducible across trials
    ...
    # train_test_CLCLSA logs mean/std – return macro‑F1 mean for Optuna
    mean_f1 = metrics["mean_f1_macro"]   # adapt if you expose differently
